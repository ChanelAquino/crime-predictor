...
treeFit = rpart(crime.type ~ area.name + victim.age + weapon.type  ,data=tr_dat, method="class")
...
```{r}
predicted = predict(treeFit, te_dat, class="class")
predicted = as.data.frame(predicted)
#y = apply(predicted,2,function(x) max(x))
#predicts = predicted[,names(y)[which.max(y)]]
