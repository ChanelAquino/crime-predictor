...
As we can see from the model above, the classification tree is only displaying three nodes.
...
For this first model, we decided to use area.name, victim.sex, premise.description and victim.descent to predict the crime.type.  The model is trained on 80% of the data and tested on the remaining 20%.
```{r}
library(e1071)
...
set.seed(123)
tr.rows = sample(1:nrow(crimes), .80*nrow(crimes))
tr.dat = crimes[tr.rows,]
te.dat = crimes[-tr.rows,]

#Creating model
nb.fit1 = naiveBayes(crime.type ~ area.name + victim.sex + premise.description + victim.descent, data=tr.dat, type='raw')
